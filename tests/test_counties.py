...
from dsgrid.dataformat import counties_filepath, load_counties, write_counties, \
    read_counties, fips_to_countyindex

# Test reading in counties from CSV, confirm matching data

countymap, counties = load_counties(counties_filepath)
...
def test_nonstandardcountywriteread():
    ...
    random_county_subset = []
    for ind in sample(range(len(counties)), 1500):
        random_county_subset.append(counties[ind])
    ...
    with TempHDF5() as testfile:
        ...
        assert [counties[i] for i in countymapping] == h5counties
        assert random_county_subset == h5counties
